...
const administrador = () => {
  ...
  const onSubmitIngreso = async () => {
    ...
    setInfo(
      Object.assign({}, info, {
        value: response.data.rackAssing,
        text: "#RACK",
      })
    );
  };
  ...
  const onSubmitSalida = async () => {
    ...
    setRacks(Object.assign({}, racks, { freeRacks: response.data.rack }));
    setInfo(
      Object.assign({}, info, { value: response.data.price, text: "PRECIO" })
    );
  };

  return (
    /*
        description: aca pondria la descripcion
        */
    <Layout toogleProfile={toogleProfile} urlImg="/img/park.png">
      ...
      {profile ? (
        ...
        <AdminProfile toogleProfile={toogleProfile} />
      )}
    </Layout>
  );
};
